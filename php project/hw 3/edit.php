...
if(isset($_GET['id'])){
$id=$_GET['id'];
$result=$con ->prepare('select * from project where id=?');
...
$sdate=$_POST['sdate'];
$edate=$_POST['edate'];
$sql=$con ->prepare('update project set pname=?,p_desc=?,status=?,s_date=?,e_date=? where id=?');
$sql->bind_param('sssssi',$pname,$desc,$status,$sdate,$edate,$id);
if($sql ->execute()){
